...
HttpServer::HttpServer(const std::string &address, unsigned short port,
    ...
    : m_ioc{1}, m_acceptor{m_ioc, tcp::endpoint{asio::ip::make_address(address),
...
HttpServer::~HttpServer() { SPDLOG_TRACE("HttpServer::~HttpServer"); }
...
void HttpServer::on_accept(beast::error_code ec, tcp::socket socket)
{
    ...
    if (!ec) {
        auto transaction = std::make_shared<HttpTransaction>(
            transactionId, std::move(socket), std::make_unique<JsonSerializer>(),
