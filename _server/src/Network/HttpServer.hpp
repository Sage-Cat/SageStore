...
/**
 * @class HttpServer
 * @brief Implements an HTTP server that accepts connections and manages HTTP
 * sessions.
 *
 * This class sets up a listening TCP socket on a specified address and port. It
 * accepts incoming connections and creates HttpTransaction objects for handling
 * each connection, leveraging the BusinessLogic for processing requests.
 */
class HttpServer {
public:
    ...
    /**
   * @brief Constructs an HttpServer with a given address, port, and reference
   * to BusinessLogic.
   *
   ...
   */
    HttpServer(const std::string &address, unsigned short port,
               ExecuteBusinessTaskCallback callback);
   ...
   */
    ...
    /**
   * @brief Starts the server, making it listen for incoming connections and
   * process requests.
   */
    void run();
...
private:
    ...
    tcp::acceptor m_acceptor; ///< Acceptor used for listening for and accepting
