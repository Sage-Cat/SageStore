...
public:
    SqliteQueryResult(sqlite3_stmt *stmtPtr) : stmt(stmtPtr, sqlite3_finalize) {}
...
public:
    ...
    Impl(std::string_view path, std::string_view createDatabaseScriptPath)
        : m_dbPath(path), m_createDatabaseScriptPath(createDatabaseScriptPath)
    {
    }
    ...
    bool open()
    {
        ...
        const auto fileExists =
            std::filesystem::exists(m_dbPath); // Use C++17 std::filesystem for file existence check
                                               ...
                                               const std::vector<std::string> &params)
    {
        ...
        if (sqlite3_prepare_v2(m_db.get(), query.data(), -1, &stmtRaw, nullptr) != SQLITE_OK) {
            ...
            throw ServerException(_M, "Failed to prepare statement: " +
        ...
        for (size_t i = 0; i < params.size(); ++i) {
            if (sqlite3_bind_text(stmtRaw, static_cast<int>(i + 1), params[i].c_str(), -1,
                                  SQLITE_TRANSIENT) != SQLITE_OK) {
                SPDLOG_ERROR("Failed to bind parameter at position {}: {}", i + 1,
                             sqlite3_errmsg(m_db.get()));
                sqlite3_finalize(stmtRaw); // Clean up before throwing
                throw ServerException(_M, "Failed to bind parameter at position " +
                                              std::to_string(i + 1) + ": " +
                                              std::string(sqlite3_errmsg(m_db.get())));
            }
        }
    ...
    bool initializeDatabaseSchema()
    {
        ...
        SPDLOG_DEBUG("Initializing database schema | using sql script: {}",
                     m_createDatabaseScriptPath);
...
SqliteDatabaseManager::SqliteDatabaseManager(std::string_view dbPath,
