...
void UserRepository::add(const User &entity)
{
    ...
    const std::string query = "INSERT INTO " + std::string(User::TABLE_NAME) + " (username, password) " +
...
void UserRepository::update(const User &entity)
{
    ...
    const std::string query = "UPDATE " + std::string(User::TABLE_NAME) + " SET username = ?, password = ?, roleId = ? WHERE id = ?;";
...
std::vector<User> UserRepository::getByField(const std::string &fieldName, const std::string &value) const
{
    SPDLOG_TRACE("UserRepository::getByField | {} = {}", fieldName, value);
    std::vector<User> users;

    const std::string query = "SELECT id, username, password, roleId FROM " + std::string(User::TABLE_NAME) + " WHERE " + fieldName + " = ?;";
...
std::vector<User> UserRepository::getAll() const
{
    ...
    const std::string query = "SELECT id, username, password, roleId FROM  " + std::string(User::TABLE_NAME) + " ;";
